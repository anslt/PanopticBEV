...
def panoptic_stats3(msk_gt, cat_gt, panoptic_pred, num_classes, _num_stuff, _mean_stat, _std_stat, num=1.5):
    # Move gt to CPU
    msk_gt, cat_gt = msk_gt.cpu(), cat_gt.cpu()
    msk_pred, cat_pred, _, iscrowd_pred = panoptic_pred
    ...
    if cat_gt.numel() > 1:
        ...
        # flag TP matches, i.e. same class and iou > 0.5
        # matches = ((cat_gt.unsqueeze(1) == cat_pred.unsqueeze(0)) & (_iou > 0.5))
        _iou_small = _iou.clone()
        _iou_pred = _iou.new_zeros(_iou.size(), dtype=torch.bool)
        _iou_small = _iou_small[1:,1:]
        row_ind, col_ind = linear_sum_assignment(-_iou_small.numpy())
        for row, col in zip(list(row_ind), list(col_ind)):
            _iou_pred[row + 1, col + 1] = True

        matches = ((cat_gt.unsqueeze(1) == cat_pred.unsqueeze(0)) & _iou_pred)
...
def compute_panoptic_test_metrics3(panoptic_pred_list, panoptic_buffer, conf_mat, **varargs):
