...
class VerticalTransformer(nn.Module):
    def __init__(self, in_ch, v_2d_ch, v_3d_ch, img_size_in, img_size_out, extents, img_scale, resolution,
                 norm_act=ABN):
    ...
    def forward(self, v_feat, intrinsics):
        ...
        v_feat = self.depth_extender_dummy(
            self.depth_extender(v_feat)).contiguous()  # Create the volumetric lattice. Shape --> N x 1 x C_2d x H x W
...
class FlatTransformer(nn.Module):
    ...
    def forward(self, feat, intrinsics):
        ...
        ipm_incorrect_fv = kornia.geometry.transform.warp_perspective(ipm_incorrect, torch.inverse(theta_ipm),
        ...
        feat_bev_ecm = F.interpolate(feat_bev_ecm, size=(int(self.Z_out), int(self.W_out)), mode="bilinear",
                                     align_corners=True)
...
class TransformerVF(nn.Module):
    def __init__(self, in_ch, tfm_ch, out_ch, extrinsics=None, bev_params=None, H_in=None, W_in=None, Z_out=None,
        ...
        self.v_transform = VerticalTransformer(in_ch=tfm_ch, v_2d_ch=tfm_ch // 2, v_3d_ch=tfm_ch // 2,
                                               img_size_in=(H_in, W_in),
